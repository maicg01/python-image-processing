...
def face_mtcnn(image):
    ...
    # boxes[:, 0] = boxes[:, 0] * ori_width / image_size[0]
    # boxes[:, 1] = boxes[:, 1] * ori_height / image_size[1]
...
if __name__ == '__main__':
    ...
    # image = cv2.imread('data/1.jpeg')
    image = cv2.imread('/home/maicg/Documents/Me/python-image-processing/Face-landmark/MTCNN/1.jpeg')
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    bboxs = face_mtcnn(image_rgb)
    # image_rs = cv2.resize(image,(128, 128))
    print(bboxs)
    
    for detection in bboxs:
        print("log 1")
