...
def txt_format_yolo(label, pil_bbox, landmarks, width, height):
...
def detect(
    ...
    path_txt = "/home/maicg/Desktop/total_train_data/coco/labels_pose" #nhan goc sau khi da detect va gan nhan keypoint = 0
    ...
    for path, im, im0s, vid_cap in dataset:
        print("---------------path-----------------", path)
        # print(path)
        file_name = os.path.basename(path).split('.jpg')[0]
        print(file_name)
        dir_txt = path_txt + '/' + str(file_name) + '.txt'
        dir_save_txt = save_txt + '/' + str(file_name) + '.txt'
        try:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for j in range(det.size()[0]):
                    ...
                    rs = txt_format_yolo(7,xyxy,landmarks,width, height)
                    # print(rs)
                    lines.append(rs)
